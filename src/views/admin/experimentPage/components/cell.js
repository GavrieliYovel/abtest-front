...
export default function Cell(props) {
    ...
    return (
        <Box marginY={"10px"} w={"30%"} display={'flex'} flexDirection={"column"} alignItems={"center"}>
            {
                rest.id  ?
                        <CopyToClipboard text={rest.id}  onCopy={() => setCopy({copied: true})}>
                            <Button onClick={removeCopied}>
                                <Text color={"#2B3674"} >{rest.title}</Text>
                                {copy.copied ? <span borderRadius="10px" style={{fontWeight:300, color:"#01B574", marginLeft:'10px',borderRadius:"10px", padding:'0px 10px 0px 10px'}}>  copied ID  </span> : null}
                            </Button>
                        </CopyToClipboard>
                    :
                        <Text color={"#2B3674"} >{rest.title}</Text>
